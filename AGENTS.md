...
- `src/cursor-provider-auth-reload.ts` owns auto-recover after Cursor auth failures and stale aborts (`/cursor-sdk-recover` → reload → continue), with a cooldown to avoid reload loops.
