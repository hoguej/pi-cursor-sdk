...
- `src/cursor-provider-auth-reload.ts` owns auto-`/reload` after Cursor auth failures (invalid/unauthorized/missing API key) with a cooldown to avoid reload loops.
