...
- `src/cursor-provider-auth-reload.ts` owns auto-`/reload` after Cursor auth failures so the existing API key is re-resolved (and a rotated key is picked up if present), with a cooldown to avoid reload loops.
